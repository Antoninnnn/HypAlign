#!/bin/bash
#SBATCH --job-name=hypalign-plip-mf-clean
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
echo "=== Clean GO-MF PLIP-BCE rerun: existing ProtST MF splits + namespace-specific NeuML text cache ==="
echo "Node: $(hostname)"
...
conda run --no-capture-output -p "$CONDA_ENV" python -u \
    ...
    --label "plip_esm2_650m_neuml_go_mf_clean_bce_posw_cap100" \
    --epochs 200 \
    --batch-size 256 \
    --embed-batch-size 16 \
    --text-batch-size 64 \
